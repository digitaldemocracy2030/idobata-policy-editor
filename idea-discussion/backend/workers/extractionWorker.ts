...
async function processExtraction(job: Job): Promise<void> {
  const { sourceType, sourceOriginId, content, metadata, themeId } = job.data;
  console.log(
    `[ExtractionWorker] Starting extraction for ${sourceType}: ${sourceOriginId}`
  );

  try {
    let llmResponse: ExtractionResponse;
    const addedProblemIds: Types.ObjectId[] = [];
    const addedSolutionIds: Types.ObjectId[] = [];
    ...
    if (sourceType === "chat") {
      ...
      llmResponse = await callLLM(extractionPromptMessages, true) as ExtractionResponse; // Request JSON
