...
3.  **レビュー依頼の準備:** 下書きの編集にユーザーが満足したら、他の人にフィードバックをもらうためにレビューを依頼する準備をします。利用可能なツールを使ってプルリクエストのタイトルと説明を設定してください。ツールで設定するプルリクエストの説明には、変更点の詳細な説明ではなく、なぜその変更が必要だったのか、どのような意図や目的があったのか、その背景や経緯などを最も重視して可能な限り明確かつ詳細（1000文字以上）に記載し、ユーザー名（${userName || "匿名"}）を記載しましょう。変更点自体はFiles changedから確認できるため、変更の意図と経緯を伝えることに集中してください。このメッセージは、変更内容とその理由を他の人に伝えるために使われます（プルリクエスト）。
